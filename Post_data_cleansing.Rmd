...
Often, the first major part -- "prepare" -- is the most time consuming. This can be lamented since many anylists prefare the cool modeling aspects (since I want to show my math!). In practice, one rather has to get his (her) hands dirt...
...
So, the dataset ist quite huge: `r dim(data)` (rows/cols)
...
That raises the question who to name the data set? At least two points are worth thinking. First, the "root" name should it be the name of the project such as "OKCupid" or "nycflights13"? Or just something like "data"? Personally, I prefer "data" as it is short and everybody will know what's going on. Second question: Should some data manipulations should be visisble in the name of the object? I, personally, like this and do that fruquently, eg., `carat_mean`, so I will remember that the mean is in there. However, for bigger projects I have made the experience that I lose track on which is the most recent version. So better do *not* put data manipulations in the name of the data frame. However, what one can do is use attributes, eg.:
...
Clearly, there are different ways to get to Rome; I present just one, which has proved hepful for me.

## Identify missings
A key point is here to address many columns in one go, otherwise it gets laborous with large data sets. Here's one way:
...
Boxplots (or violinplots/beanplots) may also be a way:
...
Puh, that takes ages to plot. Maybe better don't plot, since we are not after exploration, but just want to know if something is going wrong.
...
Plausibility check can includes checking orders of magnitude, looking for unplausible values (negative body weight), among others. A good starter is to differentiate between numeric and non-numeric variables.
...
For instance, the max. age of 110 appears somewhat high... And the min. height of 1 should rather be excluded before furhter operations start. For `income` similar reasoning applies.
...
A quite frequent use case is to get "strange" value or variables names in order, e.g., "variable 2" oder "I rather not say" (including blanks or some other non-normal stuff).
...
But wait; one issue is that the dataframe now consists of the numeric variables only. We have to manually knit it together with the rest of the party. Not so convenient. Rather, try this:
